...
- [~] Input-closure manifest + single-rename commit + cross-version replay (FR-S1–S4). **FR-S1 signed manifest done** (`manifest.py`: content-addressed run_id, sha256 over input-closure + code versions + enabled_producers; `verify()` detects tamper/skip). **FR-S3 single-rename commit done** (`commit.py`: stage → atomic rename `runs/<run_id>/` → flip `CURRENT`; `read_current()` trusts only a byte-for-byte-verified run, discards leftover `.staging`; idempotent via content-addressed run_id). **FR-S2 replay done** (`replay.py`: `plan()` classifies each cited ref frozen/requery/tamper — reuse frozen verdicts, re-query a re-versioned claim, exit 2 on unexplained snapshot drift; `check_versions()` blocks silent cross-version upgrade with a migration instruction). **FR-S4 publish gate done** (`commit.py`: a later run marks the prior run's manifest `superseded_by` — excluded from the signature so it stays valid — and `assert_publishable(run_id)` refuses to publish from a superseded/tampered run).
- [x] Exit taxonomy + coverage invariant + body re-classification backstop (FR-X1–X3, FR-R1). **FR-X1/X3 done** (exit 0/1/2 + coverage invariant in validate_ledger). **FR-R1 done** (`backstop.py`: re-classify rendered body sentences; a high-risk sentence not mapping to a `verified` claim → exit 2 — static recall becomes a runtime invariant).
- [x] `dedup.py` → frozen `independence_partition.json` (FR-I0). **Throttle/budget done** (`budget.py`: reserve-before-dispatch counter, all-or-nothing reserve, MAX_CONCURRENT/MAX_VERIFIER_CALLS/MAX_SPANS_PER_CLAIM caps; FR-P2/NFR-1).
- [x] Golden fixtures for **every** §6.7 branch (D5) — `test_gate_core` covers all branches. **Appendix B normalization fixture corpus done** (`tests/fixtures/appendix_b.jsonl` + `test_appendix_b.py`: NFKC/entities/smart-quotes/dashes/ellipsis/NBSP/case/paraphrase/fabrication/occurrence + FR-A5 re-extraction — distinguishes a normalization bug from genuine non-entailment).
...
## M3 — RAG (post-MVP)
- [~] Embeddings + rerank + hybrid + semantic dedup (FR-I1, FR-R3). **FR-I1 semantic independence done** (`semantic.py`: embedding cosine merges paraphrased syndication that lexical simhash misses; deterministic lexical-fallback embedder + pluggable real backend, NFR-5; independence only tightens; refines `independence_partition.json` → `dedup_version 0.2.0-m3`). **FR-R3 retrieval done** (`retrieval.py`: query expansion → hybrid BM25+embedding → cross-encoder rerank → top-k; deterministic BM25-lite + lexical-embedder + term-coverage reranker fallbacks, pluggable real backends, NFR-5; recall aid only — surfaced passages still pass the entailment gate).
